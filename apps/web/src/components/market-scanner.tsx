...
export function MarketScanner() {
  ...
  return <>
    ...
    <p className="form-message">Primary scan action: discover candidates for approval (explicit PRE_SCAN mode).</p>
    ...
    {results.length > 0 ? <section className="panel candidate-assessment-summary" aria-label="Candidate assessment summary"><h2>Candidate assessment summary</h2><p>{summary.tracked} tracked · {summary.processed} processed · {summary.candidates} pre-scan candidates · {summary.noTrade} no-trade · {summary.unavailable} unavailable</p><p>{Object.entries(reasonCounts).filter(([reason]) => reasonLabels[reason]).map(([reason, count]) => `${count} ${reasonLabels[reason]}`).join(" · ") || "No assessment reasons recorded"}</p><p>Pre-scan candidates are suitable for offline review and pre-approval, not currently executable. Broker readiness, fresh quotes, Greeks, liquidity, Guardian, expiry, and final risk validation are checked again before any order. No-trade and unavailable symbols remain visible here and were not silently dropped.</p></section> : null}
